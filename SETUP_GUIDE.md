...
1. Google Apps Script 편집기에서 함수 실행:
   - 왼쪽 메뉴에서 함수 선택 드롭다운 클릭
   - **`testPermissions`** 함수 선택
   - 실행 버튼(▶️) 클릭
   - **권한 승인 팝업이 나타나면 "허용" 클릭**
   - 실행 로그 확인:
     - ✅ 외부 API 호출 권한 승인 완료
     - ✅ Google Sheets 권한 승인 완료 (또는 경고 메시지)
     - ✅ Google Drive 권한 승인 완료

2. 개별 권한 테스트:
   - **`testSheetsPermission`**: Google Sheets 접근 권한만 테스트
   - **`testDrivePermission`**: Google Drive 접근 권한만 테스트

3. 권한 오류 발생 시:
   - 실행 로그에서 오류 메시지 확인
   - 권한 승인 팝업이 나타나지 않으면:
     - "배포" → "배포 관리" → 기존 배포 삭제 후 재배포
     - 또는 "프로젝트 설정" → "OAuth 범위" 확인
...
### Google Sheets/Drive 접근 권한 오류
- **증상**: "권한이 없습니다" 또는 "접근할 수 없습니다" 오류
- **해결**: 
  1. **권한 테스트 실행**:
     - GAS 편집기에서 `testPermissions()` 함수 실행
     - 권한 승인 팝업이 나타나면 "허용" 클릭
  2. **배포 설정 확인**:
     - "배포" → "배포 관리" → 기존 배포 편집
     - **"실행 대상"이 "나"로 설정되었는지 확인** ⚠️ 중요
  3. **Google Sheets 접근 권한**:
     - `testSheetsPermission()` 함수 실행
     - 해당 Google Sheet에 대한 접근 권한이 있는지 확인
     - Sheet ID가 올바른지 확인
  4. **Google Drive 접근 권한**:
     - `testDrivePermission()` 함수 실행
     - Google Drive 저장 공간이 충분한지 확인
  5. **재배포**:
     - 권한 오류가 계속되면 배포 삭제 후 재배포
     - 재배포 시 모든 권한 승인 팝업에 "허용" 클릭
